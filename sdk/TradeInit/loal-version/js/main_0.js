...
(async function () {       
  ...
  // for (let i in accounts) {
    let gasPrice = await web3.eth.getGasPrice().then(console.log);
    ...
    */
    ...
    for (i = 0; i < metamaskAccounts.length; i++) {
      ...
      // }
      let temp = await TransactionBuyInitForTEST(metamaskAccounts[i], Prices[i], TestTokenArry[i], WETHArry[i], UniswapV2Router02Arry[i]);
